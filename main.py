...
class BrowserTab(QWidget):
    ...
    """
    def lock_mouse(self):
        self.browser.page().setMousePanActive(True)
        self.browser.setCursor(Qt.CursorShape.BlankCursor)
    """
...
class BrowserWindow(QMainWindow):
    def __init__(self):
        ...
        self.add_tab_button = QAction(QIcon("plus.svg"), "New Tab", self)
        self.add_tab_button.triggered.connect(lambda: self.add_tab(QUrl('http://www.google.com'), 'New Tab'))
        self.toolbar.addAction(self.add_tab_button)
        ...
        # Create a keyboard shortcut for locking the mouse with F12
        #self.lock_mouse_shortcut = QShortcut(QKeySequence(Qt.Key_F12), self)
        #self.lock_mouse_shortcut.activated.connect(self.lock_mouse)

    def lock_mouse(self):
        self.current_tab().lock_mouse()

    def mouseReleaseEvent(self, event):
        # Release the locked mouse cursor
        #self.current_tab().unlock_mouse()
